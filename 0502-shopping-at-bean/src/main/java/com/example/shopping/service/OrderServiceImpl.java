...
//ユーザーは注文ができる=ユーザーは注文を登録できる=個人情報を登録できる+商品購入情報を登録できる(購入商品と購入に際しての合計額を登録できる)+注文できたことを知ることが出来る
//実現したい機能から、ユーザーに送ってほしいデータとそれに対応したInputが決まるというところまでは理解。しかし、送ってもらうと言ってもどこまでのデータをフロント側からサーバー側に送ってもらうかというところがいまいちつかめない。
//例えば購入商品に関して言うと、productIdだけ送ってもらって、それに対応した名前や金額をサーバー側から送るのか、フロント側で画面を表示するためにサーバー側から予め送られたデータを
//使うし、それをサーバー側に送るのかといったところ。
//とりあえず進めるか~
//やはりInputとEntityのフィールドとしてどのようなものを用意すべきかのベストプラクティスや定石がわかっていないな。
